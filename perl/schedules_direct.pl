...
schedule_direct - Grab TV listings from Schedules Direct SD-JSON service.

=head1 SYNOPSIS

schedule_direct --help

schedule_direct --info

tschedule_direct --version

schedule_direct --capabilities

schedule_direct --description


schedule_direct [--config-file FILE]
                [--days N] [--offset N]
                [--output FILE] [--quiet] [--debug]

schedule_direct --configure [--config-file FILE]
...
Run schedule_direct with the --configure option to create a config file.

MythTV does not use the default XMLTV config file path. If using MythTV you
should also specify the config file such as:

    schedule_direct --configure --config-file ~/.mythtv/source_name.xmltv
